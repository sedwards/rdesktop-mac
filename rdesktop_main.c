...
rdesktop_main(int argc, char *argv[])
{
	...
	logger(Core, Debug, "======== SESSION SETUP COMPLETE ========");
	logger(Core, Debug, "Requested session dimensions: %dx%d", g_requested_session_width, g_requested_session_height);
	logger(Core, Debug, "Window size type: %d", g_window_size_type);
	logger(Core, Debug, "DPI: %d", g_dpi);
	logger(Core, Debug, "========================================");
	...
	while (1)
	{
		...
		logger(Core, Debug, "======== PRE-CONNECTION PHASE ========");
		logger(Core, Debug, "Final session dimensions after limitations: %dx%d", g_requested_session_width, g_requested_session_height);
		logger(Core, Debug, "Server: %s", server);
		logger(Core, Debug, "Username: %s", g_username ? g_username : "(none)");
		logger(Core, Debug, "========================================");

		/* Create window BEFORE attempting connection - this ensures user can see dimensions */
		logger(Core, Debug, "======== CREATING WINDOW BEFORE CONNECTION ========");
		rd_create_ui();
		logger(Core, Debug, "Window created successfully, now attempting connection...");
		logger(Core, Debug, "========================================");

		if (!rdp_connect
		    (server, flags, domain, g_password, shell, directory, g_reconnect_loop))
		{
			logger(Core, Error, "======== CONNECTION FAILED ========");
			logger(Core, Error, "Failed to connect to %s", server);
			logger(Core, Error, "====================================");
		...
		/* Window already created before connection, just ensure it's ready */
		if (!ui_have_window())
		{
			logger(Core, Warning, "Window does not exist after connection! Creating now...");
			rd_create_ui();
		}
